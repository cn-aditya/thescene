...
class BCMAPI
{
	...
	protected $url_auth = 'oauth.brightcove.com/v3/access_token?grant_type=client_credentials';
	protected $url_request = 'cms.api.brightcove.com/v1/accounts';
	 ...
	 */
	public function __construct($token_read = NULL, $token_write = NULL, $client_id = NULL, $client_secret = NULL, $account_id = NULL)
	{
	...
	protected function getUrl($type = 'read')
	{
		...
		if(strtolower($type) == 'read')
		{
			...
			$url .= $this->url_write;
		} elseif(strtolower($type) == 'auth') {
			$url .= $this->url_auth;
		} elseif(strtolower($type) == 'request') {
	...
	protected function curlRequest($request, $get_request = FALSE)
	{
		...
		if($get_request)
		{
			...
			//curl_setopt($curl, CURLOPT_POSTFIELDS, 'json=' . $request);
		}
	 ...
	 * @return string The media asset ID
	 */
	public function createMediaViaApi($type = 'video', $paramsArr)
	{
		if(strtolower($type) == 'video')
			{
				...
				$parameters->video = $paramsArr;
				$parameters->token = $this->token_write;

				$jsonObj->params = $parameters;
				$jsonObj->method = strtolower('create_' . $type);
				$jsonObj->jsonrpc = '2.0';				

				$request = json_encode($jsonObj);
					...
					$response =  $this->bit32clean($response);
	...
	public function get_folder_list($access_token, $method = 'GET', $type = 'folders',$folder_id  = NULL, $data = NULL){
		switch($type){
			...
			break;
			case 'videos' :
			$end_point = "folders/{$folder_id}/videos";
			break;
			default:
			$end_point = "folders";
		}
